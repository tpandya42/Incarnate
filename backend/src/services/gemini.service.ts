...
export const optimizePrompt = async (
  ...
  const textPrompt = `
    ...
    2. Single character only - ONE person in the entire image
    3. Front-facing view, full body visible from head to feet
    4. T-pose or A-pose with arms slightly away from the body (for 3D rigging)
...
export const generateAvatarVideo = async (
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      console.log(`[Gemini] Video generation attempt ${attempt}/${MAX_RETRIES}`);
      ...
      while (!operation.done) {
        ...
        const metadata = operation.metadata as { state?: string } | undefined;
      ...
      const response = operation.response as {
        generatedVideos?: Array<{ video?: { uri?: string }; uri?: string }>;
      ...
      if (!downloadLink) {
        console.error('[Gemini] Full Veo response:', JSON.stringify(operation.response, null, 2));
      ...
      const isInternalError =
        lastError.message.toLowerCase().includes('internal') ||
        lastError.message.toLowerCase().includes('server');
      
      if (attempt < MAX_RETRIES && isInternalError) {
        console.warn(`[Gemini] Video generation failed (Attempt ${attempt}), retrying in 4s...`, lastError);
        await delay(4000);
        continue;
      }
      break;
      ...
      responseSchema: {
        ...
        properties: {
          pitch_range: {
            type: Type.OBJECT,
            properties: {
              min_hz: { type: Type.NUMBER },
              max_hz: { type: Type.NUMBER },
            },
          },
